...
##        print "Here goes node " + str(iters) + " out of " + str(len(root_nodes))
        ...
        for mapping in mappings_list:
            ...
            if target_tuple in freq_dict:
                ...
                freq_dict[target_tuple] = 1
        if n==135357:
              print "ROOT NODE: ",D.node[n], "matchings found: ",len(allEmbeddings)
              break
    if returnEmb:                                               ## added May2013
...
def handle_quota_Furer(D,  P, total_Zlist_dict_copy, result_to_give, iteration_counter):
    ...
    nr_emb=0
    for k in total_Zlist_dict_copy.keys():
        nr_emb+=total_Zlist_dict_copy[k]/float(iteration_counter)
    print "Nr embeddings in this quota: ",nr_emb
...
def matches_Furer(n, D,  m,  P,  matched_list,  OBmatched):
    if predicate_match(D.node[n],  P.node[m]):
        ...
        if len(test_list) !=len(OBmatched+[m]): #hence, they should be of equal size and ready to be compared
            #if not os.path.exists(os.path.join(output_path,'warning_non_equal.info')):
            #    with open(os.path.join(output_path,'warning_non_equal.info')) as f:
