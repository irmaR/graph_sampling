...
short_graph_file_name = "patt4"
pattern_file_name = "/home/irma/workspace/graph_sampling/exampleExperiment/pattern4_exp/pattern_exhaustive_final_test.gml" # BEWARE: change below ALSO the Plist, OBdecomp and root-target AND fdict_ex pickle file !!!
D = nx.read_gpickle(graph_file_name)
P = nx.read_gml(pattern_file_name)
root_nodes = [x for x in D.nodes() if D.node[x]['predicate']=='citations']
...
start=time.time()
fdict_exhaustive_inf = sampling_exhaustive_general2(D,  P,  Plist,  root_nodes,True)
stop=time.time()
...
start=time.time()
fdict_exhaustive = sampling_exhaustive_general2(D,  P,  Plist,  root_nodes,True)
stop=time.time()

print "Pickle name: ","fdict_exhaustive_%s.pickle" % short_graph_file_name
